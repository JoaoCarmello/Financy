        ...
        <div className="flex flex-row gap-6">
            ...
            <Table>
              ...
              <TableBody>
                {dataT.listTransactions.length > 0 && dataT.listTransactions.slice(0, 5).map((transaction) => {
                  const IconCategory =
                    Icons[transaction?.category?.icon as keyof typeof Icons] || Icons.BriefcaseBusiness
          ...
          <Card className="p-0">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead colSpan={2} className="font-medium text-gray-500 uppercase p-6">categories</TableHead>
                  <TableHead className="">
                    <Link to="/categories">
                      <Button
                        size="sm"
              ...
              <TableBody>
                {dataC.listCategories.length > 0 && dataC.listCategories.map((category) => {
                  return (
                    <TableRow key={category.id}>
                      <TableCell className="text-right p-6 font-semibold flex flex-row items-center gap-4 ">
                        <CardTitle className={`${colorVariants[category.color]} w-fit rounded-xl px-3 py-1 font-medium text-sm`}>{category.name}</CardTitle>
                      </TableCell>

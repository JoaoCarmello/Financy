...
export function Categories() {
  ...
  const mostFrequentCategory = data?.listCategories.reduce((max, category) => {
  ...
  const Icon = Icons[response.mostFrequentCategory?.icon as keyof typeof Icons] || Icons.Question;
    ...
    <>
      <div>
        ...
        <div className="flex flex-row justify-between gap-6 mt-8 mb-8">
          ...
          <Card className="uppercase flex flex-row items-start justify-left p-6 w-full">
            <Icon size={24} className={`text-${response.mostFrequentCategory?.color}-base`} />
            <div className="flex flex-col gap-2">
              <CardTitle className="font-bold text-3xl text-gray-800">{response.mostFrequentCategory?.name || 'Nenhuma'}</CardTitle>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {response.category.map((category) => {
            const Icon = Icons[category?.icon as keyof typeof Icons] || Icons.BriefcaseBusiness

            return (
              <Card className="p-6">
                <div className="flex flex-row items-center justify-between">
                  <div className={`h-10 w-10 rounded-xl flex items-center justify-center p-3 ${colorVariants[category.color]}`}>
                    <Icon size={16} />
                  </div>
                  <div className="flex flex-row items-center justify-between gap-2">
                    <Button variant="outline" className="text-danger" onClick={() => handleDeleteCategory(category.id)}>
                      <Trash size={24} />
                    </Button>
                    <Button variant="outline" className="text-gray-700" onClick={() => handleEditCategory(category)}>
                      <SquarePen size={24} />
                    </Button>
                  </div>
                </div>
                ...
                <div className="flex flex-row items-center justify-between">
                  <CardTitle className={`${colorVariants[category.color]} rounded-xl px-3 py-1 font-medium text-sm`}>{category.name}</CardTitle>
                  <CardDescription className="text-gray-600 text-sm">{category.countTransactions} item{category.countTransactions || 0 > 1 ? 's' : ''}</CardDescription>
                </div>
              </Card>
            )
          })}
